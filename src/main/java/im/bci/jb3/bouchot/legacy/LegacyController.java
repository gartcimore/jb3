...
public class LegacyController {
    ...
    @RequestMapping(value = "/anatid/plop/from", method = RequestMethod.POST)
    public void plopFrom(@RequestParam(value = "id", required = false) String postId,
            @RequestParam(value = "time", required = false) String time,
            @RequestParam(value = "info", required = false) String info,
            @RequestParam(value = "login", required = false) String login,
            @RequestParam(value = "message") String message,
            @RequestParam(value = "tribune", required = false) String tribune) {
        GatewayPostId gatewayPostId = new GatewayPostId();
        gatewayPostId.setGateway(tribune);
        gatewayPostId.setPostId(postId);
        if (!postPepository.existsByGatewayPostId(gatewayPostId)) {
            Post post = new Post();
            post.setGatewayPostId(gatewayPostId);
            post.setRoom(tribune);
            DateTime postTimeRounded = LegacyUtils.legacyPostTimeFormatter.parseDateTime(time).secondOfMinute()
                    .roundFloorCopy();
            ...
            String nickname = CleanUtils.truncateNickname(login);
            if (StringUtils.isBlank(nickname)) {
                nickname = CleanUtils.truncateNickname(info);
            }
            post.setNickname(CleanUtils.cleanNickname(nickname));
            post.setMessage(legacyUtils.convertFromLegacyNorloges(
                    CleanUtils.cleanMessage(CleanUtils.truncateMessage(message)), post.getTime(), tribune));
