...
public class HadokenGateway implements Gateway {
    ...
    public void post(String nickname, String message) {
        try {
            Jsoup.connect("http://hadoken.free.fr/board/post.php").data("message", legacyUtils.convertToLegacyNorloges(message, DateTime.now().withZone(LegacyUtils.legacyTimeZone).secondOfMinute().roundFloorCopy())).userAgent(nickname).data("last_id", String.valueOf(lastPostId)).parser(Parser.xmlParser()).post();
        } catch (IOException ex) {
            Logger.getLogger(HadokenGateway.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
